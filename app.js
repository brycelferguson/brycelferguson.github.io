...
selPubs = [];
selPubs.push(publications["Journal Articles"]["j7"]);
selPubs.push(publications["Journal Articles"]["j8"]);
selPubs.push(publications["Journal Articles"]["j4"]);
selPubs.push(publications["Peer-Reviewed Conference Articles"]["c10"]);
for (idx in selPubs) {
    pub = selPubs[idx];
    if (idx<4) {
...
for (keyIdx in Object.keys(publications)) {
    ...
    for (pubIdx in publications[key]) {
		...
		if (pub.url == "./publications/blank.html"){
			if (pub.status == "working") {
				pubsHTML+='<li style="margin-bottom: 15px">'+pub.title+'<br>'+pub.authors+', '+pub.date+'.</li>';
			} else {
				pubsHTML+='<li style="margin-bottom: 15px">'+pub.title+'<br>'+pub.authors+'<br><i>'+pub.conferencelong+'</i> ('+pub.status+'), '+pub.date+'.</li>';
			}
		} else {
			if (pub.status == "working") {
			pubsHTML+='<li style="margin-bottom: 15px"><a class="text-capitalize" href="'+pub.url+'" target="_blank">'+pub.title+'</a><br>'+pub.authors+', '+pub.date+'. [<a class="text-capitalize" href="'+pub.pdfFile+'" target="_blank">PDF</a>]</li>';
		} else {
			pubsHTML+='<li style="margin-bottom: 15px"><a class="text-capitalize" href="'+pub.url+'" target="_blank">'+pub.title+'</a><br>'+pub.authors+'<br><i>'+pub.conferencelong+'</i> ('+pub.status+'), '+pub.date+'. [<a class="text-capitalize" href="'+pub.pdfFile+'" target="_blank">PDF</a>]</li>';
		}
		}
    }
    pubsHTML += "</ul>";
...
for (idx in news) {
    ...
    if (news[idx].url == "") {
        ...
        newsItm += " [<a href=" + news[idx].url + " target=\"_blank\">Link</a>]</p></div>";
    }
